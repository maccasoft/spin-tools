...
data: ('DAT' | 'dat')+ NL* (org | orgh | directive | dataLine)* ;
 
org
    : ('ORG' | 'org') (address=expression (',' limit=expression)? )? NL+ (directive | dataLine)*
    ;

orgh
    : ('ORGH' | 'orgh') (address=expression (',' limit=expression)? )? NL+ (directive | dataLine)*
    ;
...
label
    : {_input.LT(1).getCharPositionInLine() == 0}? '.'? VARIABLE
    ;
