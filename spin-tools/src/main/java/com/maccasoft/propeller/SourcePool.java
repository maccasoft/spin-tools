...
public class SourcePool {
    ...
    public void renameSource(File oldFile, File newFile) {
        String text = sources.remove(oldFile);
        if (text != null) {
            changeSupport.firePropertyChange(oldFile.getAbsolutePath(), text, null);
            sources.put(newFile, text);
            changeSupport.firePropertyChange(newFile.getAbsolutePath(), null, text);
        }
    }
