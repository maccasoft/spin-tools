...
public class Testn extends Spin1PAsmInstructionFactory {
    ...
    public Spin1InstructionObject createObject(Spin1PAsmLine line) {
        if (Spin1PAsmSchema.D_S_WC_WZ.check(line.getArguments(), line.getEffect())) {
            ...
            return new Testn_(line.getScope(), line.getCondition(), line.getArguments().get(0), line.getArguments().get(1), line.getEffect());
        }
    ...
    public Spin1InstructionObject createObject(Spin1Context context, String condition, List<Spin1PAsmExpression> arguments, String effect) {
        if (Spin1PAsmSchema.D_S.check(arguments, effect)) {
            return new Testn_(context, condition, arguments.get(0), arguments.get(1), effect);
        }
        throw new RuntimeException("Invalid arguments");
    }

    /*
     * TESTN   D,{#}S   {WC/WZ/NR/WR}
     */
    public class Testn_ extends Spin1InstructionObject {
        ...
        public Testn_(Spin1Context context, String condition, Spin1PAsmExpression dst, Spin1PAsmExpression src, String effect) {
            super(context);
        ...
        // 011001_000i_1111_ddddddddd_sssssssss

        @Override
        public byte[] getBytes() {
            int value = instr.setValue(encodeInstructionParameters(condition, dst, src, null), 0b011001);
