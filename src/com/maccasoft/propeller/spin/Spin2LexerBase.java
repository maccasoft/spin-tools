...
public abstract class Spin2LexerBase extends Lexer {
    ...
    protected Spin2LexerBase(CharStream input) {
        super(input);
    }
    ...
    public Token nextToken() {
        ...
        if (_input.LA(1) == EOF && _indents.size() > 0) {
            if (_buffer[_lastTokenInd] == null || _buffer[_lastTokenInd].getType() != Spin2Lexer.NL) {
    ...
    protected void HandleNewLine() {
        //emit(Spin2Lexer.NL, HIDDEN, getText());
    ...
    protected void HandleSpaces() {
        ...
        if ((_lastToken == null || _lastToken.getType() == Spin2Lexer.NL) && IsNotNewLineOrComment(next)) {
        ...
        emit(Spin2Lexer.WS, HIDDEN, getText());
    }
